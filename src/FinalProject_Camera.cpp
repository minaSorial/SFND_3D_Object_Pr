...
int main(int argc, const char *argv[])
{
  ...
  //ofstream myfile;
  //myfile.open ("../result/result.txt");
  //myfile << "Detector , Descriptor , imgID, TTC Lidar, TTC Camera , Diffrence "<<endl;
            ...
            for (auto it1 = (dataBuffer.end() - 1)->bbMatches.begin(); it1 != (dataBuffer.end() - 1)->bbMatches.end(); ++it1)
            {
                ...
                if (currBB->lidarPoints.size() > 0 && prevBB->lidarPoints.size() > 0) // only compute TTC if we have Lidar points
                {
                    ...
                    if (bVis)
                    {
                        ...
                        //string name = detectorType + " | " + descriptorType +imgNumber.str()+".jpg";  

                        //cv::imwrite("../result/"+name, visImg);
                   ...
                   //myfile <<detectorType<<","<<descriptorType<<","<<imgIndex<<","<<ttcLidar<<","<<ttcCamera<<","<< ttcCamera-ttcLidar<< endl;
                    //cout<<"img "<<imgIndex << " ttcCamera  "+ detectorType + "/" + descriptorType +" = " << ttcCamera <<  " s,  ttcLidar = " <<ttcLidar << " s" <<  ", dif ="<< ttcCamera-ttcLidar<< endl;
